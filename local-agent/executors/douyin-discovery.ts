...
import { extractTopicCandidates as extractTopics, type TopicCandidate, type TopicRuleOptions } from "../../lib/topic-extractor.ts";
import type { CrawlerTaskResult, CrawlerTaskSnapshot, CrawlerTaskStatus, DiscoveryMode, StartDouyinDiscoveryInput } from "../contracts.ts";
import { agentDataRoot, crawlerPythonCommand, crawlerRuntimePath, mediaCrawlerRoot, projectRoot } from "../runtime-paths.ts";
...
function taskOutputBase(task: RunningCrawlerTask): string {
  return path.resolve(agentDataRoot(), "mediacrawler-runs", task.id);
}

function jsonlDir(task: RunningCrawlerTask): string {
  return path.join(taskOutputBase(task), "douyin", "jsonl");
}
...
function pushLog(task: RunningCrawlerTask, chunk: Buffer | string) {
  const lines = outputLines(chunk);

  task.logs.push(...lines);
  task.logs = task.logs.slice(-180);
}

function outputLines(chunk: Buffer | string): string[] {
  return String(chunk)
    // Rich/Typer and Python logging may emit terminal formatting sequences.
    // They make API error messages unreadable and can break the dashboard layout.
    .replace(/\u001b\][^\u0007]*(?:\u0007|\u001b\\)/g, "")
    .replace(/\u001b\[[0-?]*[ -/]*[@-~]/g, "")
    .split(/\r?\n/)
    .map((line) => line.trim())
...
function crawlerFailureMessage(code: number | null, stderrLines: string[]): string {
  ...
  const summary = `Crawler failed, exit code: ${code ?? "unknown"}`;
...
function buildArgs(keywords: string, maxNotes: number, task?: RunningCrawlerTask): string[] {
  ...
  if (task?.id) args.push("--save_data_path", taskOutputBase(task));
  return args;
}
...
async function hasDouyinVerificationChallenge(): Promise<boolean> {
  ...
  try {
    ...
    return targets.some((target) =>
      /verifycenter|nocaptcha|验证码|人机验证/i.test(`${target.title || ""} ${target.url || ""}`)
    );
  } catch {
    return false;
  }
...
function spawnCrawler(task: RunningCrawlerTask, keywords: string, stage: "keyword" | "topic") {
  ...
  const runnerScript = path.resolve(projectRoot(), "scripts", "run-mediacrawler.py");
  const outputRoot = taskOutputBase(task);
  ...
  task.command = `${pythonCommand} ${runnerScript} ${args.join(" ")}`;
  task.status = "running";
  ...
  void acquireCdpTaskLock({
    ...
    try {
      ...
      const child = spawn(pythonCommand, [runnerScript, ...args], {
        cwd: dir,
        shell: false,
        windowsHide: true,
        env: {
          ...
          no_proxy: "localhost,127.0.0.1,::1",
          PLAYWRIGHT_BROWSERS_PATH: path.join(process.env.LOCALAPPDATA || path.dirname(agentDataRoot()), "ms-playwright"),
          PYTHONIOENCODING: "utf-8",
          PYTHONUTF8: "1",
          PYTHONDONTWRITEBYTECODE: "1",
          MEDIACRAWLER_ROOT: dir,
          MEDIACRAWLER_OUTPUT_ROOT: outputRoot,
          KOL_CRM_KEEP_CREATOR_IDENTIFIERS: "1",
          MEDIACRAWLER_DY_PUBLISH_TIME_TYPE: String(mapPublishWindowToDouyinType(task.publishWindowDays))
      ...
      child.stderr.on("data", (chunk) => {
        stderrLines.push(...outputLines(chunk));
      ...
      child.on("close", (code) => {
        void (async () => {
          ...
          const collectedItems = readRecentItems(task);
          if (code === 0 && stage === "keyword" && !collectedItems.length && await hasDouyinVerificationChallenge()) {
            task.status = "failed";
            task.stage = "done";
            task.finishedAt = new Date().toISOString();
            task.error = "抖音触发了人机验证。请在自动打开的抖音 Chrome 窗口完成验证后重新运行任务。";
          ...
          if (code !== 0) {
            ...
            task.status = "failed";
            task.finishedAt = new Date().toISOString();
            task.error = crawlerFailureMessage(code, stderrLines);
            pushLog(task, task.error);
            return;
          }
...
export function getDouyinCrawlerTaskResult(): CrawlerTaskResult {
  const task = currentTask();
  return {
    task: getDouyinCrawlerTask(),
    content: readRecentItems(task).map((item) => JSON.stringify(item)).join("\n")
  };
}

export function startDouyinCrawlerTask(input: StartDouyinDiscoveryInput): CrawlerTaskSnapshot {
